...
def load_accounts():
    ...
    for account in sql_accounts:
        account = Account(
            id=account[0],
            summoner_username=account[1],
            region=account[2],
            username=account[3],
            password=encryption.decrypt_message(account[4]),
        )
    return accounts
...
class Account:
    ...
    def fetch_summoner_web_data(self):
        if self.summoner_username == "":
            ...
            return
        return tf(
            webscraper.fetch_account_data,
            self.summoner_username,
            self.region,
            None,
            self,
        )
    ...
    def save(self):
        ...
        database.save_to_table(
            "account",
            ("id", "summoner_username", "region", "username", "password"),
            (self.id, self.summoner_username, self.region, self.username, encrypted),
        )
