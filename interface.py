...
def create_account_list_layout():
    ...
    layout_header = [
        [
            gui.Text(header[0], size=header[1], background_color="Gray")
    ...
    for count, ph in enumerate(placeholders):
        ph.append(
            gui.Button("Login", size=(10, 1), key=f"-login-{count + 1}-", visible=False)
        )
        ph.append(
            gui.Button(
                "Delete", size=(10, 1), key=f"-delete-{count + 1}-", visible=False
            )
        )
    ...
    buttons = [
        [
            gui.Button("Add Account", size=(10, 1), key="-add-account-"),
            ...
            if not account.id in loaded_ids:
                for counter in range(50):
                    if window[f"-ID-{counter + 1}-"].get() == "":
                        ...
                        window[f"-Name-{counter + 1}-"].update(
                            account.summoner_username
                        )
                        window[f"-Region-{counter + 1}-"].update(account.region)

                        if account.data is not None:
                            window[f"-Level-{counter + 1}-"].update(
                                account.data["level"]
                            )
                            window[f"-Rank-{counter + 1}-"].update(account.data["rank"])
                            window[f"-Games 30D-{counter + 1}-"].update(
                                str(account.data["games_played"]) + " games"
                            )
                            window[f"-Hours Played-{counter + 1}-"].update(
                                str(round(account.data["time_played_minutes"] / 60, 2))
                                + " hours"
                            )
                            total_games += int(account.data["games_played"])

                        window["-games-played-"].update(
                            str(total_games) + " matches played in the last 30 days"
                        )

                        login_dict[f"-login-{counter + 1}-"] = (
                            account,
                            counter + 1,
                            acc_index,
                        )
                        delete_dict[f"-delete-{counter + 1}-"] = (
                            account,
                            counter + 1,
                            acc_index,
                        )
                ...
                if account.data is not None:
                    window[f"-Level-{loaded_ids[account.id]}-"].update(
                        account.data["level"]
                    )
                    window[f"-Rank-{loaded_ids[account.id]}-"].update(
                        account.data["rank"]
                    )
                    window[f"-Games 30D-{loaded_ids[account.id]}-"].update(
                        str(account.data["games_played"]) + " games"
                    )
                    window[f"-Hours Played-{loaded_ids[account.id]}-"].update(
                        str(round(account.data["time_played_minutes"] / 60, 2))
                        + " hours"
                    )
...
def add_account_logic(window, event, values):
    ...
    if event == "-submit-":
        if values["-summoner-"] != "":
            account_page = webscraper.fetch_account(
                values["-summoner-"], values["-region-"]
            )
            if not account_page:
        ...
        account = accounts.Account(
            id=database.count_from_table("account", "id")[0][0],
            summoner_username=values["-summoner-"],
...
def main(accounts=None):
    ...
    layout = [
        [gui.Column(create_account_list_layout(), key="-account-list-")],
        [
            gui.Column(
                create_add_account_layout(), key="-add-account-menu-", visible=False
            )
        ],
        [
            gui.Column(
                create_change_path_layout(), key="-change-path-menu-", visible=False
            )
        ],
    ]
